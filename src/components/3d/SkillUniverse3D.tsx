...
const SKILL_NODES: SkillNode[] = [
  // Programming & Core
  { name: 'C++', category: 'Programming', pos: [0, 0, 0], color: '#00E5FF' },
  { name: 'DSA', category: 'Programming', pos: [1.2, 0.6, 0.4], color: '#A3FF12' },
  { name: 'Python', category: 'Programming', pos: [-1.1, 0.8, -0.3], color: '#38BDF8' },
  { name: 'TypeScript', category: 'Programming', pos: [0.8, -0.9, 0.5], color: '#FBBF24' },

  // Frontend Cluster
  { name: 'React', category: 'Frontend', pos: [-1.8, -0.8, 1.2], color: '#00E5FF' },
  { name: 'Tailwind CSS', category: 'Frontend', pos: [-2.2, 0.5, 0.8], color: '#06B6D4' },
  { name: 'Three.js', category: 'Frontend', pos: [-1.5, -1.8, 0.6], color: '#A855F7' },

  // Backend Cluster
  { name: 'Node.js', category: 'Backend', pos: [1.9, 1.2, -0.8], color: '#22C55E' },
  { name: 'FastAPI', category: 'Backend', pos: [2.3, -0.4, -0.7], color: '#10B981' },
  { name: 'Express.js', category: 'Backend', pos: [1.4, 1.8, 0.2], color: '#94A3B8' },

  // Database Cluster
  { name: 'PostgreSQL', category: 'Database', pos: [0.4, 2.1, -1.1], color: '#3B82F6' },
  { name: 'PostGIS', category: 'Database', pos: [-0.6, 1.9, -1.4], color: '#6366F1' },

  // AI/ML Cluster
  { name: 'Machine Learning', category: 'AI / ML', pos: [-0.5, -2.0, -1.2], color: '#EC4899' },
  { name: 'Computer Vision', category: 'AI / ML', pos: [0.7, -2.2, -0.8], color: '#8B5CF6' },
  { name: 'Docker', category: 'Tools', pos: [2.1, -1.6, 0.9], color: '#0284C7' },
];
            ...
            <Html distanceFactor={8} position={[0, 0.32, 0]} center>
              <div
                ...
                className={`cursor-pointer px-2 py-0.5 rounded text-[10px] font-mono whitespace-nowrap border transition-all duration-200 ${
                  isHovered || isActive
                    ? 'bg-[#00E5FF]/20 border-[#00E5FF] text-white shadow-[0_0_12px_#00E5FF]'
                    : 'bg-[#0B1120]/80 border-slate-700/60 text-slate-300 hover:text-white'
                }`}
              >
                {skill.name}
              </div>
...
export const SkillUniverse3D: React.FC<{
  ...
  return (
    <div className="relative w-full h-[480px] rounded-2xl overflow-hidden border border-[#00E5FF]/20 bg-[#080D1A] shadow-[inset_0_0_40px_rgba(0,0,0,0.8)]">
      <Canvas
      ...
      {/* Mini HUD Info Tag */}
      <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[11px] font-mono glass-panel px-3 py-1.5 rounded-lg border border-slate-800 text-slate-300">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-[#00E5FF] animate-ping" />
          <span>SELECTED: {activeSkill ? activeSkill.name : 'ALL SYSTEMS'}</span>
        </div>
        <span className="text-[#00E5FF] text-[10px]">3D INTERACTIVE CONSTELLATION</span>
      </div>
    </div>
  );
};
