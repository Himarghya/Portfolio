...
const getCategoryIcon = (category: string) => {
  switch (category) {
    case 'Programming': return <Code2 className="w-4 h-4 text-[#00FF87]" />;
    case 'Frontend': return <Layout className="w-4 h-4 text-[#6EE7B7]" />;
    case 'Backend': return <Server className="w-4 h-4 text-[#10B981]" />;
    case 'Database': return <Database className="w-4 h-4 text-[#34D399]" />;
    case 'AI / ML': return <Brain className="w-4 h-4 text-[#A3FF12]" />;
    case 'Tools': return <Wrench className="w-4 h-4 text-[#059669]" />;
    default: return <Cpu className="w-4 h-4 text-[#00FF87]" />;
  }
};
    ...
    <section id="skills" className="relative py-24 border-t border-slate-900 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex items-center gap-2 glass-panel p-1.5 rounded-xl border border-emerald-950">
            <button
              onClick={() => setViewMode('grid')}
              className={`px-3.5 py-1.5 rounded-lg text-xs font-mono transition-all ${
                viewMode === 'grid'
                  ? 'bg-[#00FF87]/20 text-[#00FF87] border border-[#00FF87]/50 shadow-[0_0_12px_rgba(0,255,135,0.2)]'
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-xs font-mono tracking-wider transition-all duration-200 border ${
                activeCategory === cat
                  ? 'bg-[#00FF87]/15 text-[#00FF87] border-[#00FF87]/60 shadow-[0_0_15px_rgba(0,255,135,0.15)] font-semibold'
                  : 'bg-[#040E08]/80 text-slate-400 border-emerald-950 hover:border-emerald-800 hover:text-white'
              }`}
            >
              {cat.toUpperCase()}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          <AnimatePresence>
            {filteredSkills.map((skill, index) => (
              ...
              >
                <div className="glass-panel p-5 rounded-xl border border-emerald-950 group-hover:border-[#00FF87]/40 group-hover:bg-[#061B10]/90 transition-all duration-300 h-full flex flex-col justify-between">
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center gap-2.5">
                        <div
                          className="p-2 rounded-lg bg-[#020704] border border-emerald-900/60 group-hover:border-[#00FF87]/30 transition-colors"
                          style={{ color: skill.color }}
                        >
                          {getCategoryIcon(skill.category)}
                        </div>
                        <div>
                          <h3 className="font-mono font-bold text-sm text-white group-hover:text-[#00FF87] transition-colors">
                            {skill.name}
                          </h3>
                  ...
                  {/* Tags */}
                  <div className="pt-3 border-t border-emerald-950 flex flex-wrap gap-1.5">
                    {skill.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 rounded text-[10px] font-mono bg-white/5 text-slate-300 border border-emerald-950"
                      >
                        {tag}
                      </span>
                    ))}
