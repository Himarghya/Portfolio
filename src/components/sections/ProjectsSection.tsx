        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono text-[#00FF87] uppercase tracking-widest">// SHOWCASE</span>
            </div>
          ...
          <div className="glass-panel px-4 py-2.5 rounded-xl border border-emerald-950 text-xs font-mono text-slate-400">
          ...
          {PROJECTS.map((project, index) => {
            ...
            return (
              <motion.div
                ...
                className="relative rounded-2xl border border-emerald-950 hover:border-[#00FF87]/40 bg-gradient-to-b from-[#040E08] to-[#020704] p-6 sm:p-8 lg:p-10 shadow-2xl transition-all duration-300"
              >
                {/* Cyber Corner Decals */}
                ...
                <div className={`grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center ${isEven ? '' : 'lg:flex-row-reverse'}`}>
                  ...
                  <div className={`lg:col-span-6 space-y-5 ${isEven ? 'lg:order-2' : 'lg:order-1'}`}>
                    ...
                    <div className="flex items-center gap-3">
                      <span className="font-mono font-black text-2xl sm:text-3xl text-emerald-900">
                        {project.number}
                      </span>
                      <span className="px-3 py-1 rounded-full text-xs font-mono border border-emerald-900/60 bg-[#020704] text-emerald-300">
                        {project.category}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs sm:text-sm font-mono text-[#00FF87]">
                        // {project.subtitle}
                      </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {project.techStack.map((tech) => (
                        <span
                          key={tech}
                          className="px-2.5 py-1 rounded-md text-xs font-mono bg-[#020704] border border-[#00FF87]/20 text-emerald-300"
                        >
                          {tech}
                        </span>
                      ))}
                    ...
                    {/* Action Links */}
                    <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-emerald-950">
                      {project.githubUrl && (
                        <a
                          href={project.githubUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-mono bg-[#020704] border border-emerald-900/60 text-slate-200 hover:text-[#00FF87] hover:border-[#00FF87]/50 transition-all"
                        >
                      ...
                      {project.liveUrl && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-mono bg-[#00FF87]/15 border border-[#00FF87]/50 text-[#00FF87] hover:bg-[#00FF87]/25 transition-all"
                        >
                          <span>LIVE DEMO</span>
