  ...
  return (
    <section id="terminal" className="relative py-24 border-t border-slate-900 overflow-hidden">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <HudBadge label="SECTION // 06" variant="cyan" pulse={false} />
              <span className="text-xs font-mono text-[#00E5FF] uppercase tracking-widest">// CLI INTERFACE</span>
            </div>
            <h2 className="text-3xl sm:text-5xl font-bold tracking-tight text-white">
              CODE. BUILD. <span className="text-glow-cyan text-[#00E5FF]">REPEAT.</span>
            </h2>
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#0B1120] border border-[#00E5FF]/30 text-white hover:text-[#00E5FF] hover:border-[#00E5FF]/70 text-xs font-mono transition-all shadow-lg"
            >
        ...
        {/* Terminal Window */}
        <div className="rounded-2xl border border-[#00E5FF]/30 bg-[#050A14] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.8)]">
          ...
          <div className="px-4 py-3 bg-[#0B1120] border-b border-slate-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[10px] font-mono text-slate-400 hover:text-white"
                title="Copy developer spec"
              >
                {copied ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                <span>{copied ? 'COPIED' : 'COPY SPEC'}</span>
              </button>
              <span className="text-[10px] font-mono text-[#00E5FF] hidden sm:inline">UTF-8 // READY</span>
            </div>
          </div>
          ...
          >
            {/* Intro text */}
            <div className="text-slate-500 text-xs leading-relaxed border-b border-slate-800/60 pb-3">
              Antigravity Quantum Shell v2.4 (x86_64-quantum-linux)<br />
              Type <span className="text-[#00E5FF]">"help"</span> for commands, or explore <span className="text-[#A3FF12]">"projects"</span>, <span className="text-[#38BDF8]">"skills"</span>, <span className="text-[#EC4899]">"neofetch"</span>.
            </div>
            ...
            {history.map((item) => (
              <div key={item.id} className="space-y-1.5">
                <div className="flex items-center gap-2 text-slate-400">
                  <span className="text-[#00E5FF] font-bold">himarghya@core:~$</span>
            ...
            <form onSubmit={handleCommand} className="flex items-center gap-2 pt-2">
              <span className="text-[#00E5FF] font-bold shrink-0">himarghya@core:~$</span>
              <input
                ref={inputRef}
                type="text"
                value={inputVal}
                ...
                className="w-full bg-transparent text-white font-mono focus:outline-none placeholder-slate-600 caret-[#00E5FF]"
                autoComplete="off"
                spellCheck={false}
              />
              <button type="submit" className="text-slate-600 hover:text-[#00E5FF]">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4 text-xs font-mono">
          ...
          {['help', 'neofetch', 'skills', 'projects', 'contact', 'clear'].map((cmd) => (
            <button
              ...
              className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-slate-400 hover:text-[#00E5FF] hover:border-[#00E5FF]/40 transition-colors"
            >
              ${cmd}
            </button>
          ))}
