...
const getMilestoneIcon = (icon: string) => {
  switch (icon) {
    case 'Terminal': return <Terminal className="w-5 h-5 text-[#00E5FF]" />;
    case 'Layers': return <Layers className="w-5 h-5 text-[#38BDF8]" />;
    case 'Brain': return <Brain className="w-5 h-5 text-[#EC4899]" />;
    case 'Globe': return <Globe className="w-5 h-5 text-[#A3FF12]" />;
    ...
    <section id="journey" className="relative py-24 border-t border-slate-900 overflow-hidden">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center text-center mb-16 space-y-3">
          <HudBadge label="SECTION // 05" variant="purple" pulse={false} />
          <h2 className="text-3xl sm:text-5xl font-bold tracking-tight text-white">
            MY <span className="text-glow-purple text-[#A78BFA]">JOURNEY</span>
          </h2>
        ...
        <div className="relative">
          ...
          {/* Glowing Center Line */}
          <div className="absolute left-4 sm:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-[#00E5FF] via-[#7C3AED] to-[#A3FF12] transform sm:-translate-x-1/2 opacity-40 shadow-[0_0_15px_rgba(0,229,255,0.4)]" />
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-4 sm:left-1/2 transform -translate-x-1/2 flex items-center justify-center z-20">
                    <div className="w-10 h-10 rounded-full bg-[#05070D] border-2 border-[#00E5FF] shadow-[0_0_20px_rgba(0,229,255,0.5)] flex items-center justify-center">
                  ...
                  <div className={`pl-12 sm:pl-0 sm:w-1/2 ${isEven ? 'sm:pr-12' : 'sm:pl-12'}`}>
                    <div className="glass-panel p-6 rounded-xl border border-slate-800 hover:border-[#00E5FF]/40 transition-all duration-300 relative group">
                      ...
                      {/* Milestone Title */}
                      <h3 className="text-lg font-bold font-mono text-white mb-2 group-hover:text-[#00E5FF] transition-colors">
                        {item.title}
                      </h3>
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-2 border-t border-slate-800/80">
                        {item.skills.map((skill) => (
                          <span
                            key={skill}
                            className="px-2 py-0.5 rounded text-[10px] font-mono bg-white/5 border border-slate-800 text-slate-300"
                          >
                            {skill}
                          </span>
                        ))}
